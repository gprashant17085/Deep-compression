...
import argparse
from func.models import LeNet
import util
import os

os.makedirs('savedModel', exist_ok=True)

parser = argparse.ArgumentParser(description='Deep compression on LeNet using MNIST dataset')
...
torch.save(model, f"savedModel/initialTrainedLeNetmodel.ptmodel")
...
train(args.epochs)
torch.save(model, f"savedModel/prunedAndtrainedModel.ptmodel")
accuracy = test()
